...
function clicked(e){
    ...
    if(myturn == 0){//黒のターンなら
        let flipped = getFlipCells(i,j,BLACK);//ひっくり返す番号を取得（二次元配列で返って来る　例: [[2,3],[2,4],[2,5]]）
        ...
        let flipped = getFlipCells(i,j,WHITE);//ひっくり返す番号を取得（二次元配列で返って来る　例: [[2,3],[2,4],[2,5]]）
        console.table("flipped : " +flipped);
        //flippedに入っている配列に従ってput関数を使ってコマをおいていく
        if(flipped.length > 0){
            for(let k = 0 ; k < flipped.length; k++){
...
function canFlip(color){
    for(let x = 0; x < 8; x++){
        for(let y = 0 ; y < 8;y++){//二重ループで全盤面確認
            let flipped = getFlipCells(x,y, color);//指定したコマで何個ひっくり返すか
